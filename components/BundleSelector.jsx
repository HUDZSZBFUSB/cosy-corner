...
export default function BundleSelector({ product, variant, onVariantChange }) {
  ...
  // Stock stable par produit (2-5 unités)
  const stock = useMemo(() => {
    const hash = product.handle.split("").reduce((a, c) => a + c.charCodeAt(0), 0);
    return (hash % 4) + 2; // 2, 3, 4 ou 5
  }, [product.handle]);
    ...
    <div className="mt-6 w-full">
      ...
      <div className="mt-4 flex items-center gap-2">
        ...
        <p className="text-[13px] text-neutral-700">
          <span className="font-bold text-red-600">Plus que {stock} en stock</span>
          {" "}— commandez avant qu'il soit trop tard.
        </p>
      </div>

      <button
        onClick={add}
        className="btn-atc mt-3 w-full rounded-full py-5 font-bold tracking-wider uppercase text-[15px] flex items-center justify-center gap-3 transition"
      >
